...
export default function Reports({companyName,bwRate,colorRate}:ReportsProps){
 ...
 const totalPages=rows.reduce((t,r)=>t+r.pages_printed,0),totalAnomalies=rows.reduce((t,r)=>t+r.anomaly_count,0),fixedCost=rows.filter(r=>r.cost_source==="fixed_monthly").reduce((t,r)=>t+r.estimated_cost,0),variableCost=rows.filter(r=>r.cost_source!=="fixed_monthly"&&r.cost_source!=="not_applicable").reduce((t,r)=>t+r.estimated_cost,0),totalCost=fixedCost+variableCost;
 ...
 return <div className="reports-page">
  ...
  <section className="reports-metrics"><article><span>Tarifa P&B</span><strong>{loading?"...":rate(bwRate)}</strong><small>contratual por página</small></article><article><span>Tarifa colorida</span><strong>{loading?"...":rate(colorRate)}</strong><small>aplicada quando houver contador de cor</small></article><article><span>Custo fixo</span><strong>{loading?"...":currency(fixedCost)}</strong><small>equipamentos com cobrança mensal</small></article><article><span>Total consolidado</span><strong>{loading?"...":currency(totalCost)}</strong><small>{number(totalPages)} páginas no período</small></article></section>
  <section className="reports-table-card"><div className="reports-table-title"><div><small>DETALHAMENTO POR EQUIPAMENTO</small><h3>Fechamento individual das impressoras</h3></div><span>{rows.length} item(ns) · {totalAnomalies} anomalia(s)</span></div><div className="reports-table-scroll"><table className="reports-table reports-table-financial"><thead><tr><th>Impressora</th><th>IP</th><th>Unidade</th><th>Setor</th><th>Modelo</th><th>Inicial</th><th>Final</th><th>Impressões</th><th>R$/pág.</th><th>Custo estimado</th></tr></thead><tbody>{!loading&&rows.map(row=>{const label=isLabelPrinter(row);return <tr key={row.printer_uuid}><td><strong>{row.display_name}</strong><small>{row.serial||row.hostname||"Sem identificação extra"}</small></td><td>{row.ip||"-"}</td><td>{row.unit_name||"-"}</td><td>{row.sector_name||"-"}</td><td>{modelLabel(row.manufacturer,row.model)}</td><td>{number(row.opening_page_count)}</td><td>{number(row.closing_page_count)}</td><td className="pages">{label?"N/A":number(row.pages_printed)}</td><td className="rate">{label?"N/A":row.cost_source==="fixed_monthly"?"Fixo mensal":rate(row.cost_per_page)}<small>{label?"impressora de etiquetas":row.cost_source==="fixed_monthly"?currency(row.fixed_monthly_cost)+"/mês":row.cost_source==="printer"?"específica":"padrão empresa"}</small></td><td className="cost">{label?"Não aplicável":currency(row.estimated_cost)}</td></tr>})}{!loading&&rows.length===0&&<tr><td colSpan={10} className="reports-empty">Nenhum histórico encontrado para este período.</td></tr>}</tbody></table></div>{loading&&<div className="reports-loading">Carregando relatório...</div>}</section>
 </div>
}
