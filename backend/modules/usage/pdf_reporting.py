...
def build_pdf_report(company_name: str, start: date, end: date, rows: list[dict], report_scope: str = "Parque completo") -> bytes:
    ...
    doc = SimpleDocTemplate(output, pagesize=page_size, leftMargin=8*mm, rightMargin=8*mm, topMargin=6*mm, bottomMargin=9*mm, title="Printflow - Relatorio de Impressao")
    styles = getSampleStyleSheet()
    title = ParagraphStyle("pf-title-v3", parent=styles["Title"], fontName="Helvetica-Bold", fontSize=14.5, leading=15, alignment=1, textColor=colors.HexColor(f"#{BRAND_NAVY}"), spaceAfter=0)
    brand = ParagraphStyle("pf-brand-v3", parent=styles["Normal"], fontSize=6.8, leading=7.4, textColor=colors.HexColor(f"#{BRAND_MUTED}"))
    meta = ParagraphStyle("pf-meta-v3", parent=styles["Normal"], fontSize=6.8, leading=7.5, textColor=colors.HexColor("#24364B"))
    meta_center = ParagraphStyle("pf-meta-center-v3", parent=meta, alignment=1)
    meta_right = ParagraphStyle("pf-meta-right-v3", parent=meta, alignment=2)
    cell = ParagraphStyle("pf-cell-v3", parent=styles["Normal"], fontSize=5.15, leading=5.55, textColor=colors.HexColor("#142638"), wordWrap="LTR")
    cell_right = ParagraphStyle("pf-cell-right-v3", parent=cell, alignment=2)
    ...
    ]], colWidths=[67*mm,147*mm,67*mm], rowHeights=[11*mm])
    ...
    ]], colWidths=[93*mm,95*mm,93*mm], rowHeights=[9*mm])
    ...
    table=Table(data,repeatRows=1,colWidths=[36*mm,36*mm,42*mm,40*mm,19*mm,19*mm,23*mm,25*mm,27*mm],rowHeights=[5.7*mm]+[4.8*mm]*len(rows),hAlign="CENTER")
    table.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor(f"#{BRAND_BLUE}")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,0),5.5),("ALIGN",(0,0),(3,0),"LEFT"),("ALIGN",(4,0),(-1,0),"RIGHT"),("ALIGN",(4,1),(-1,-1),"RIGHT"),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("GRID",(0,0),(-1,-1),0.2,colors.HexColor("#B8C7D5")),("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white,colors.HexColor("#F5F8FB")]),("LEFTPADDING",(0,0),(-1,-1),2.4),("RIGHTPADDING",(0,0),(-1,-1),2.4),("TOPPADDING",(0,0),(-1,-1),.7),("BOTTOMPADDING",(0,0),(-1,-1),.7)]))
    ...
    metric = ParagraphStyle("pf-metric-v3", parent=meta, fontSize=6.3, leading=7, textColor=colors.HexColor(f"#{BRAND_MUTED}"), alignment=1)
    summary=Table([[
        Paragraph(f"EQUIPAMENTOS NO FECHAMENTO<br/><font size='10.5' color='#{BRAND_NAVY}'><b>{len(rows)}</b></font>",metric),
        Paragraph(f"IMPRESSÕES APLICÁVEIS<br/><font size='10.5' color='#{BRAND_NAVY}'><b>{_num(total_pages)}</b></font>",metric),
        Paragraph(f"CUSTO ESTIMADO<br/><font size='10.5' color='#{BRAND_NAVY}'><b>{_money(total_cost)}</b></font>",metric),
    ]],colWidths=[93*mm,95*mm,93*mm],rowHeights=[10*mm])
    summary.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),colors.HexColor("#F6F9FC")),("BOX",(0,0),(-1,-1),.35,colors.HexColor("#C8D7E5")),("INNERGRID",(0,0),(-1,-1),.25,colors.HexColor("#C8D7E5")),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("LEFTPADDING",(0,0),(-1,-1),2),("RIGHTPADDING",(0,0),(-1,-1),2),("TOPPADDING",(0,0),(-1,-1),.8),("BOTTOMPADDING",(0,0),(-1,-1),.8)]))

    note = Paragraph("* Impressoras de etiquetas são exibidas como N/A e não entram no volume/custo por página.", ParagraphStyle("pf-note-v3",parent=meta,fontSize=5.7,leading=6.2,textColor=colors.HexColor(f"#{BRAND_MUTED}")))
    story=[header,Spacer(1,1.2*mm),info,Spacer(1,1.8*mm),table,Spacer(1,1.8*mm),summary,Spacer(1,.8*mm),note]
