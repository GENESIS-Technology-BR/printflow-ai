...
def _fixed_cost_for_period(monthly_cost: float, start: date, end: date) -> float:
    """Prorate a monthly contract across the exact report period."""
...
def _apply_cost_models(rows: list[dict], printers: list[Printer], start: date, end: date) -> list[dict]:
    ...
    for row in rows:
        ...
        if _is_label_printer(printer):
            row["cost_per_page"] = 0.0; row["estimated_cost"] = 0.0; row["cost_source"] = "not_applicable"
            continue
        if (printer.cost_model or "per_page") == "fixed_monthly" and printer.fixed_monthly_cost is not None:
            row["cost_per_page"] = 0.0
            row["estimated_cost"] = _fixed_cost_for_period(float(printer.fixed_monthly_cost), start, end)
            row["cost_source"] = "fixed_monthly"
    return rows
...
def _validate_report_filters(db: Session, company_id: int, printer_uuid: str | None = None, unit_name: str | None = None, sector_name: str | None = None) -> None:
    if printer_uuid and db.query(Printer.id).filter(Printer.company_id == company_id, Printer.uuid == printer_uuid, Printer.active.is_(True)).first() is None:
        raise HTTPException(status_code=404, detail="Impressora nao encontrada para esta empresa.")
    if unit_name and db.query(Printer.id).filter(Printer.company_id == company_id, Printer.unit_name == unit_name, Printer.active.is_(True)).first() is None:
        raise HTTPException(status_code=404, detail="Unidade nao encontrada para esta empresa.")
    if sector_name and db.query(Printer.id).filter(Printer.company_id == company_id, Printer.sector_name == sector_name, Printer.active.is_(True)).first() is None:
        raise HTTPException(status_code=404, detail="Setor nao encontrado para esta empresa.")
...
def _report_data(db: Session, current_user: User, start: date, end: date, printer_uuid: str | None = None, unit_name: str | None = None, sector_name: str | None = None):
    ...
    default_cost = 0
    if company:
        default_cost = company.default_bw_cost_per_page or company.default_cost_per_page
...
def _report_rows(db: Session, current_user: User, start: date, end: date, printer_uuid: str | None = None, unit_name: str | None = None, sector_name: str | None = None) -> list[dict]:
    rows, _ = _report_data(db, current_user, start, end, printer_uuid, unit_name, sector_name)
    return rows
...
def usage_report(start_date: date | None = None, end_date: date | None = None, printer_uuid: str | None = None, unit_name: str | None = None, sector_name: str | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    start, end = _resolve_period(start_date, end_date)
    return _report_rows(db, current_user, start, end, printer_uuid, unit_name, sector_name)
...
def export_usage_excel(start_date: date | None = None, end_date: date | None = None, printer_uuid: str | None = None, unit_name: str | None = None, sector_name: str | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    start, end = _resolve_period(start_date, end_date)
    rows, history = _report_data(db, current_user, start, end, printer_uuid, unit_name, sector_name)
    company = db.query(Company).filter(Company.id == current_user.company_id).first(); company_name = company.name if company else "Empresa"
    content = build_excel_report(company_name, start, end, rows, history, report_scope=_report_scope_label(rows, printer_uuid, unit_name, sector_name))
...
def export_usage_pdf(start_date: date | None = None, end_date: date | None = None, printer_uuid: str | None = None, unit_name: str | None = None, sector_name: str | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    rows, _ = _report_data(db, current_user, start, end, printer_uuid, unit_name, sector_name)
    company = db.query(Company).filter(Company.id == current_user.company_id).first(); company_name = company.name if company else "Empresa"
    content = build_pdf_report(company_name, start, end, rows, report_scope=_report_scope_label(rows, printer_uuid, unit_name, sector_name))
